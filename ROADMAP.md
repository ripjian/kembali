...
| 2026-07-11 | **Design-exploration sandbox on branch `design-explorations`** (not merged, not linked from the real site). Ten full landing-page concepts at `/concepts/1`–`/10` plus an index at `/concepts`, each a distinct direction (1 kopitiam skeuomorphism, 2 dark glassmorphism, 3 motion scrollytelling, 4 neo-brutalist poster, 5 editorial zine, 6 shop-stories photography, 7 kinetic type specimen, 8 bento product tour, 9 dimensional 3D card, 10 conversational landing) over the same real Kembali content. `COMPARISON.md` on the branch rates each on first impression, mobile feel, conversion clarity and build cost. Installed three helper skills (stop-slop, caveman, impeccable) with `PRODUCT.md`/`DESIGN.md` seeded for impeccable. Brand rules held even in the sandbox: Pandan palette as the base of every direction, coral = earned / pandan = action, WCAG AA verified with `@kembali/core`, SOP 3 + stop-slop copy (no em dashes, no AI-tell). Concept 6 uses clearly-labelled CSS placeholder imagery (production needs licensed or own photography). One concept per commit; nothing here changes shipped scope | Founder-requested exploration of louder visual directions without touching the live marketing pages; kept isolated on its own branch so ideas can be harvested later without a revert |
