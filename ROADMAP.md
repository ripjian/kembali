...
| 2026-07-11 | **Outlets are first-class now (light implementation)**: the attribution + billing unit. Cashier picks a serving outlet once/day (skipped for single-outlet tenants), remembered in a tenant+date cookie; every stamp/point/redemption records `outlet_id` (migration 0010; `point_events.outlet_id` added). Transactions history + reports gain an outlet column + filter (hidden for single-outlet). Per-branch staff permissions + cross-outlet analytics stay Phase 6 | Founder call — attribution/billing needs the outlet on every event before Phase 6 depth. Fallback to the first outlet keeps single-outlet flows unchanged; RLS blocks cross-tenant attribution |
| 2026-07-11 | **Address moves from tenant → outlets** (adds `postcode`); migration 0011 backfills each tenant's first outlet non-destructively, `tenants.address_*` deprecated. **Merchant creation reworked into sections** (General → Plan & modules → Program → Outlets → Owner); plan preselects its module set (all-on today), outlets repeat with an "Add another" block. Platform-admin RLS policy added to `outlets` (migration 0012) so the directory can read locations | Founder review — outlets own their address as the billing unit; sectioned form scales to multi-outlet onboarding |
...
| 2026-07-19 | **Branded app plan approved (APP.md) and Capacitor chosen as the stack.** Customer-only app for chains, one codebase pinned per tenant; the shell wraps the existing `/app` PWA (server.url) plus native capability in later stages. Expo/React Native stays the fallback if store review or a client demands native rebuild; Flutter ruled out (no code share with the TS monorepo) | Weeks not months to a first branded app; every card improvement ships to web and app together; the white-label pipeline is a JSON config per brand. Apple 4.2 webview risk mitigated by Stage 3 native work (push, offline, deep links) |
| 2026-07-19 | **Stage 1 delivered as the /your-app interactive demo, UAT'd end to end; Stage 2 scaffold at `apps/mobile`** (brand-driven `capacitor.config.ts`, `check-brands` build gate, runbook). Native platforms blocked on Xcode/CocoaPods/Android Studio landing on the dev machine; production app blocked on OTP provider + domain | Design-first keeps the demo as both sales tool and spec; the scaffold makes the first real build a one-command step once toolchains exist |
| 2026-07-19 | **Marketing rebrand: Sprout palette + Zain/Nunito type**, chosen by the founder from a 144-combo design lab on real components. Near-black canvas, cartoon-green earned accent (`#58CC02`; `#337A03` for AA small text), sunny `#FFC800` progress, Zain display + Nunito body, IBM Plex Mono kept. Dark-on-green rule everywhere white would fail contrast. Marketing surface ONLY: `/app` + `/admin` keep Pandan pending a founder scope decision; Pandan originals kept as comments in `showcase.css` per founder | Founder wanted a look beyond the local palette; the role grammar (primary acts, accent earns, progress fills) survives the recolour, so product rules like "stamps wear the earned colour" read the same |
| 2026-07-19 | **Sprout rebrand reverted same day (founder).** The marketing surface returns to Pandan + Fraunces/Jakarta via git revert; the design lab stays deleted. The Sprout decision row above stands as history | Founder saw it applied and preferred the original; the lab and both decisions remain reproducible from git history |
