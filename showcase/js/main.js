/* Kembali showcase · "Ten visits"
   Vanilla, no dependencies. Raw WebGL2 hero, scroll-mapped narrative,
   canvas ink system, live phone demo. */
...
document.querySelectorAll('.reveal-line').forEach((el) => {
  ...
  if (!staggerParents.has(p)) staggerParents.add(p);
});
...
const gl = heroCanvas.getContext('webgl2', { antialias: false, alpha: false });
...
void main(){
  ...
  // slow breathing zoom + scroll zoom
  ...
  // gentle heat shimmer across the whole frame
  cuv += (fbm(cuv*6. + uTime*.08) - .5) * 0.0035;

  vec2 suv = cuv + .5;
  // when the panel is wider than the photo, look at the upper part (the pull)
  if (rs > ts) suv.y += (0.62 - 0.5) * (1.0 - ts/rs);
  vec3 col = texture(uTex, suv).rgb;

  // rising steam: two layers of drifting fbm, strongest mid-upper frame
...
/* ============================== QR inject ============================== */
fetch('assets/qr-join.svg').then(r => r.text()).then(svg => {
  document.getElementById('posterQr').innerHTML = svg;
}).catch(() => {});
...
const hudStamps = [...hudGrid.children];
let hudN = 0;
function setHud(n) {
  n = clamp(n, 0, 10);
  if (n === hudN) return;
...
}, { threshold: 0.35 }).observe(document.getElementById('joinPhone'));
...
const bcStamps = [...bcGrid.children];
...
function setRitual(p) {
  ...
  ledgerRows.forEach((r, i) => {
    ...
    // small screens: sliding 4-row window ending at the next upcoming row
...
// slow ken burns drift on the active ritual photo
function ritualDrift(p) {
  const active = ritualPhotos.find(ph => ph.classList.contains('show'));
  if (!active || reduceMotion) return;
  const drift = p * 100;
  active.style.transform = `scale(${1.12 - p * .08}) translateY(${drift * -.3}px)`;
}

/* ============================== reward ink burst + countdown ============================== */
...
function burst() {
  ...
  for (let i = 0; i < 46; i++) {
    ...
    // each particle is an irregular ink blob: 3-5 lobes
    const lobes = [];
    const n = 3 + Math.floor(Math.random() * 3);
    for (let k = 0; k < n; k++) {
      lobes.push({
        dx: (Math.random() - .5) * 1.6,
        dy: (Math.random() - .5) * 1.6,
        rr: .45 + Math.random() * .75,
      });
    }
    burstParts.push({
      x: cx, y: cy,
...
function drawBurst(now) {
  ...
  inkCtx.beginPath(); inkCtx.arc(cx, cy, ringR, 0, 7); inkCtx.stroke();
  // ink blobs
...
function kineticReward() {
  ...
  // 0 when section top hits bottom of viewport, 1 when it reaches 25% height
...
let rewardSeen = false, countdownTimer = null;
...
new IntersectionObserver(([e]) => {
  if (e.isIntersecting && !rewardSeen) {
    ...
    setTimeout(() => redeemedStamp.classList.add('stamped'), reduceMotion ? 0 : 1600);
  }
...
function startCountdown() {
  ...
  countdownTimer = setInterval(() => {
    ...
    couponMetaSpan.textContent = `Expires in ${m}:${s}`;
  }, 1000);
}
...
function pushFeedRow() {
  ...
  feedIdx++;
  const first = dashFeed.querySelector('.df-row');
  dashFeed.insertBefore(row, first);
...
const stamps = []; // {x, y (doc coords), rot, size, seed}
function sizeInk() {
...
function drawStamp(s, age) {
  const { x, y, rot, size } = s;
  const sy = y - scrollY;
  if (sy < -80 || sy > innerHeight + 80) return;
  const d = devicePixelRatio;
  ictx.save();
  ictx.translate(x * d, sy * d);
  ictx.rotate(rot);
  ...
  ictx.lineWidth = size * .18;
  // ring with a rubber-stamp gap
  ictx.beginPath();
  ictx.arc(0, 0, size * .68, s.seed, s.seed + Math.PI * 1.86);
  ictx.stroke();
  ictx.beginPath();
  ictx.arc(0, 0, size * .26, 0, 7);
  ictx.fill();
  ictx.restore();
}
...
addEventListener('pointerdown', (e) => {
  if (e.target.closest('a, button, .btn, .chip-cta')) return;
  stamps.push({
...
const parallaxEls = [...document.querySelectorAll('[data-parallax]')].map((el) => ({
  el, speed: parseFloat(el.dataset.parallax), top: 0, h: 0,
}));
...
function frameBody(now) {
  ...
  const p = clamp((scrollY - rTop) / rH, 0, 1);
  setRitual(p);
  ritualDrift(p);

  // HUD logic (hidden during the pinned ritual on small screens: the big card is on screen)
  const showHud = scrollY > innerHeight * .55 && !(mqSmall.matches && p > 0 && p < 1);
  hud.classList.toggle('show', showHud);
  let n = 0;
  if (scrollY + innerHeight * .5 > joinSection.offsetTop) n = 1;
  if (p > .05) n = 1 + Math.min(8, ritualRows);
  if (scrollY + innerHeight * .5 > rewardEl.offsetTop) n = 10;
  setHud(n);
